...
static void get_file_with_type(list_t *all, list_t **list, int type)
{
    file_t *file;
    ...
    while (all != NULL) {
        ...
        if (file->type == type)
            my_append_to_list(list, all->data);
        all = all->next;
    }
}
...
{
    int print_filepath = (flags.list[R_UPPER] || files != NULL);
...
void my_ls(list_t *files, flag_t flags)
{
    ...
    if (flags.list[D_LOWER]) {
        ...
        get_file_with_type(files, &file_list, FILE_TYPE);
        get_file_with_type(files, &dir_list, DIR_TYPE);
